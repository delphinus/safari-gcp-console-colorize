(() => {
    ...
    const colorize = (selector, options) => {
        ...
        return phrases.some(phrase => {
            ...
            const toColorize = useRegexp ? RegExp(text).test(projectNameValue) :
                projectNameValue.includes(text);
    ...
    window.setInterval(
        () => Events.call({
            action: 'get-options'
        }, options => selectors.some(selector => colorize(selector, options))),
        200);
})();
